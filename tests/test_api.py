...
class APITestCase(unittest.TestCase):
    """
    Тестирование RESTfulAPI с помощью тестового клиента
    """
    def setUp(self):
    ...
    def test_no_auth(self):
        response = self.client.get(url_for(
            'api.get_posts'), content_type='application/json'
        )
    ...
    def test_bad_auth(self):
        ...
        # Аутентификация с плохим паролем
        response = self.client.get(url_for(
            'api.get_posts'), headers=self.get_api_headers('john@example.com', 'dog')
        )
    ...
    def test_token_auth(self):
        ...
        # Выдает запрос с плохим маркером
        response = self.client.get(
        ...
        # Получить маркер
        response = self.client.get(url_for(
            'api.get_token'), headers=self.get_api_headers('john@example.com', 'cat')
        )
        self.assertTrue(response.status_code == 200)
        json_response = json.loads(response.data.decode('utf-8'))
    ...
    def test_unconfirmed_account(self):
        # Добавляет неподтвержденного пользователя
        r = Role.query.filter_by(name='User').first()
        self.assertIsNotNone(r)
        ...
        # Получает список постов с неподтвержденного аккаунта
        response = self.client.get(
    ...
    def test_posts(self):
        ...
        # Получает список постов от пользователя как подписчик
        response = self.client.get(
    ...
    def test_users(self):
        ...
        u2 = User(
            email='susan@example.com', username='susan',
    ...
    def test_comments(self):
        ...
        # Добавить другой коммент
        comment = Comment(body='Thank you!', author=u1, post=post)
        db.session.add(comment)
        db.session.commit()

        # Получить два коммента с поста
        response = self.client.get(
